...
async def on_guild_join(guild):
    ...
    msg = await channel.send("""To start a game of Hunt The Wumpus, type `htw!play` in `hunting-general`.
...
@bot.command(pass_context=True, help="Ends a hunt immediately. Only usable within a hunt channel.")
async def forfeit(ctx):
    pass
    
@bot.command(pass_context=True, help="Creates a hunt. Only usable withing the hunting general channel.")
async def play(ctx):
    with open(str(ctx.guild.id) + ".json", "r") as read_file:
        data = json.load(read_file)

        channel = await ctx.guild.create_text_channel(ctx.message.author.name + "'s Hunt", category=bot.get_channel(data["category_id"]))
